...
- **조건**: 마지막 입력 이후 **7초 이상** 무입력 **+** 그 직전 5초 안에 입력이 있었을 것
- "이전 5초엔 입력이 있었어야 한다" 가드: 페이지 로드 직후 아무 입력 없이 7초 가만 있어도 발화되지 않는다. 갑자기 멈춘 경우만.
...
[browser] [TypingOverIt] [C/침묵] 갑자기 멈췄습니다 — 마지막 입력 이후 7003ms 경과 (임계 7000ms)
...
export const TRIGGER = {
  ...
  silence:   { silenceMs: 7000, priorActivityWindowMs: 5000, historyWindowMs: 12000 },
...
| **C 침묵** | silence 타이머 사라짐 | 의미 있는 `keydown` 1회 이상 → 그로부터 7초 침묵 + "직전 5초 안에 다른 입력" 가드 재충족 |
...
5. 에디터 한 번 치고 7초 손 떼기 → `[C/침묵]` 자동 발화
...
10. **자동 발화 우선순위 테스트** (자동은 여전히 규칙 적용됨): Backspace 5회/초로 `[B]` 발화 → hold 중 7초 침묵 대기 → `[교체] backspace → silence` (C가 우선순위 더 높아서 interrupt)
